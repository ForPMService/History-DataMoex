...
public static class MegaAlertsFuturesMapper
{
    private const string Category = MappingCategories.MegaAlertsFutures;

    public static MegaAlertsFutures Map(
        MegaAlertsFuturesDTO dto,
        string secId,
        MapContext ctx,
        ...
        int rowIndex = 0)
    {
        try
        {
            ...
            // Каноническая строка: secId|beginUtcTicks|source|assetCode|alertType|threshold|value|reference
            string canonical = string.Create(
                CultureInfo.InvariantCulture,
                $"{secId}|{beginUtc.Ticks}|{ctx.SourceCode}|{RowHashHelper.Fmt(dto.AssetCode)}|{RowHashHelper.Fmt(dto.AlertType)}|{RowHashHelper.Fmt(dto.Threshold)}|{RowHashHelper.Fmt(dto.Value)}|{RowHashHelper.Fmt(dto.Reference)}");

            ulong rowHash = RowHashHelper.Compute(canonical.AsSpan());

            return new MegaAlertsFutures
            {
                SecId = secId,
    ...
    public static List<MegaAlertsFutures> MapBatch(
        IReadOnlyList<MegaAlertsFuturesDTO> dtos,
        string secId,
        MapContext ctx,
        ILogger logger,
        CancellationToken ct = default)
    {
        ...
        var result = new List<MegaAlertsFutures>(dtos.Count);
