...
public static class Hi2FuturesMapper
{
    private const string Category = MappingCategories.Hi2Futures;

    public static Hi2Futures Map(
        Hi2FuturesDTO dto,
        string secId,
        MapContext ctx,
        ...
        int rowIndex = 0)
    {
        try
        {
            ...
            // Каноническая строка: secId|beginUtcTicks|source|assetCode|metric|value|reference
            string canonical = string.Create(
                CultureInfo.InvariantCulture,
                $"{secId}|{beginUtc.Ticks}|{ctx.SourceCode}|{RowHashHelper.Fmt(dto.AssetCode)}|{RowHashHelper.Fmt(dto.Metric)}|{RowHashHelper.Fmt(dto.Value)}|{RowHashHelper.Fmt(dto.Reference)}");

            ulong rowHash = RowHashHelper.Compute(canonical.AsSpan());

            return new Hi2Futures
            {
                SecId = secId,
    ...
    public static List<Hi2Futures> MapBatch(
        IReadOnlyList<Hi2FuturesDTO> dtos,
        string secId,
        MapContext ctx,
        ILogger logger,
        CancellationToken ct = default)
    {
        ...
        var result = new List<Hi2Futures>(dtos.Count);
