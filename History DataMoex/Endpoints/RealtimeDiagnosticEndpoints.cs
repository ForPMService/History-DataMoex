...
namespace History_DataMoex.Endpoints
{
    ...
    public static class RealtimeDiagnosticEndpoints
    {
        ...
        {
            var group = routes.MapGroup("/debug/realtime/poll");
            ...
            {
                var pauseError = ValidatePauseMs(pauseMs);
                if (pauseError is not null) return pauseError;

                var r1 = await client.GetOrderbookStockAsync(ticker, ct);
                await Task.Delay(pauseMs, ct);
                var r2 = await client.GetOrderbookStockAsync(ticker, ct);
                ...
                CancellationToken ct = default) =>
            {
                ...
                var r1 = await client.GetOrderbookFuturesAsync(ticker, ct);
                await Task.Delay(pauseMs, ct);
                var r2 = await client.GetOrderbookFuturesAsync(ticker, ct);
                ...
                CancellationToken ct = default) =>
            {
                ...
                var r1 = await client.GetTradesStockAsync(ticker, cancellationToken: ct);
                await Task.Delay(pauseMs, ct);
                var r2 = await client.GetTradesStockAsync(ticker, cancellationToken: ct);

                var snap1 = BuildTradesSnapshot(
                    r1.Rows.Count, r1.DataVersion,
                    r1.Rows.Count > 0 ? r1.Rows[0].TradeNo : null,
                    r1.Rows.Count > 0 ? r1.Rows[^1].TradeNo : null,
                    null, null);

                var snap2 = BuildTradesSnapshot(
                    r2.Rows.Count, r2.DataVersion,
                    r2.Rows.Count > 0 ? r2.Rows[0].TradeNo : null,
                    r2.Rows.Count > 0 ? r2.Rows[^1].TradeNo : null,
                    null, null);

                var tradeNos1 = r1.Rows.Select(t => t.TradeNo).Where(n => n.HasValue).Select(n => n!.Value).ToHashSet();
                var tradeNos2 = r2.Rows.Select(t => t.TradeNo).Where(n => n.HasValue).Select(n => n!.Value).ToList();
                ...
                CancellationToken ct = default) =>
            {
                ...
                var r1 = await client.GetTradesFuturesAsync(ticker, cancellationToken: ct);
                await Task.Delay(pauseMs, ct);
                var r2 = await client.GetTradesFuturesAsync(ticker, cancellationToken: ct);
                ...
                CancellationToken ct = default) =>
            {
                ...
                var r1 = await client.GetCandlesTodayStockAsync(
                    ticker, tradeDate, interval: 1, cancellationToken: ct);
                await Task.Delay(pauseMs, ct);
                var r2 = await client.GetCandlesTodayStockAsync(
                    ticker, tradeDate, interval: 1, cancellationToken: ct);

                return Results.Ok(BuildCandlesReport(r1, r2, pauseMs));
            });

            // ─── Candles Today Futures ──────────────────────────
                ...
                CancellationToken ct = default) =>
            {
                ...
                var r1 = await client.GetCandlesTodayFuturesAsync(
                    ticker, tradeDate, interval: 1, cancellationToken: ct);
                await Task.Delay(pauseMs, ct);
                var r2 = await client.GetCandlesTodayFuturesAsync(
                    ticker, tradeDate, interval: 1, cancellationToken: ct);
        ...
        private static OrderbookPollReport BuildOrderbookReport(
            ...
            int pauseMs)
        {
            var bids1 = r1.Rows.Where(r => r.BuySell == "B").ToList();
            var asks1 = r1.Rows.Where(r => r.BuySell == "S").ToList();
            var bids2 = r2.Rows.Where(r => r.BuySell == "B").ToList();
            var asks2 = r2.Rows.Where(r => r.BuySell == "S").ToList();

            var snap1 = new OrderbookSnapshot(
            ...
            var snap2 = new OrderbookSnapshot(
