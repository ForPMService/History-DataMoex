...
public class FuturesTradeStatsMapperTests
{
    ...
    public void MapBatch_DefaultDtoList_DoesNotThrow()
    {
        ...
        {
            MakeValidFuturesTradeStatsDto("2026-05-07", "10:05:00", "SiM5"),
        };
    ...
    {
        var dto = MakeValidFuturesTradeStatsDto("2026-05-07", "10:05:00", "SiM5");
        var ctx = MakeContext("MOEX_ALGOPACK", "Europe/Moscow");
        TimeZoneInfo tz = SourceTimezones.Resolve(ctx.SourceTimezone);

        FuturesTradeStats5m model = FuturesTradeStatsMapper.Map(dto, "SiM5", ctx, tz);
    ...
    public void Map_HashStability()
    {
        ...
        FuturesTradeStats5m m1 = FuturesTradeStatsMapper.Map(dto, "SiM5", ctx, tz);
        FuturesTradeStats5m m2 = FuturesTradeStatsMapper.Map(dto, "SiM5", ctx, tz);
    ...
    public void Map_DifferentSource_DifferentHash()
    {
        ...
        FuturesTradeStats5m m1 = FuturesTradeStatsMapper.Map(dto, "SiM5", ctxAlg, tz);
        FuturesTradeStats5m m2 = FuturesTradeStatsMapper.Map(dto, "SiM5", ctxIss, tz);
    ...
    {
        var dto = MakeMinimalFuturesTradeStatsDto("2026-05-07", "10:05:00", "SiM5");
        var ctx = MakeContext("MOEX_ALGOPACK", "Europe/Moscow");
        TimeZoneInfo tz = SourceTimezones.Resolve(ctx.SourceTimezone);
    ...
    public void Map_DifferentLoadJobId_SameHash()
    {
        ...
        FuturesTradeStats5m m1 = FuturesTradeStatsMapper.Map(dto, "SiM5", ctx1, tz);
        FuturesTradeStats5m m2 = FuturesTradeStatsMapper.Map(dto, "SiM5", ctx2, tz);
    ...
    {
        var dto = MakeValidFuturesTradeStatsDto("2026-05-07", "10:05:00", "BRM5");
        var ctx = MakeContext("MOEX_ALGOPACK", "Europe/Moscow");
        TimeZoneInfo tz = SourceTimezones.Resolve(ctx.SourceTimezone);

        var ex = Assert.Throws<MappingValidationException>(() =>
            FuturesTradeStatsMapper.Map(dto, "SiM5", ctx, tz, rowIndex: 3));

        Assert.Equal("mapping_validation", ex.ErrorCategory);
        Assert.Equal(MappingCategories.FuturesTradeStats, ex.Category);
