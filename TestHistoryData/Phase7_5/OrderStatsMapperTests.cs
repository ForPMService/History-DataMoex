...
public class OrderStatsMapperTests
{
    ...
    public void MapBatch_DefaultDtoList_DoesNotThrow()
    {
        ...
        {
            MakeValidOrderStatsDto("2026-05-07", "10:05:00", "SBER"),
        };
    ...
    {
        var dto = MakeValidOrderStatsDto("2026-05-07", "10:05:00", "SBER");
        var ctx = MakeContext("MOEX_ALGOPACK", "Europe/Moscow");
        TimeZoneInfo tz = SourceTimezones.Resolve(ctx.SourceTimezone);

        OrderStats5m model = OrderStatsMapper.Map(dto, "SBER", ctx, tz);
    ...
    public void Map_HashStability()
    {
        ...
        OrderStats5m m1 = OrderStatsMapper.Map(dto, "SBER", ctx, tz);
        OrderStats5m m2 = OrderStatsMapper.Map(dto, "SBER", ctx, tz);
    ...
    public void Map_DifferentSource_DifferentHash()
    {
        ...
        OrderStats5m m1 = OrderStatsMapper.Map(dto, "SBER", ctxAlg, tz);
        OrderStats5m m2 = OrderStatsMapper.Map(dto, "SBER", ctxIss, tz);
    ...
    {
        var dto = MakeMinimalOrderStatsDto("2026-05-07", "10:05:00", "SBER");
        var ctx = MakeContext("MOEX_ALGOPACK", "Europe/Moscow");
        TimeZoneInfo tz = SourceTimezones.Resolve(ctx.SourceTimezone);
    ...
    public void Map_DifferentLoadJobId_SameHash()
    {
        ...
        OrderStats5m m1 = OrderStatsMapper.Map(dto, "SBER", ctx1, tz);
        OrderStats5m m2 = OrderStatsMapper.Map(dto, "SBER", ctx2, tz);
    ...
    {
        var dto = MakeValidOrderStatsDto("2026-05-07", "10:05:00", "GAZP");
        var ctx = MakeContext("MOEX_ALGOPACK", "Europe/Moscow");
        TimeZoneInfo tz = SourceTimezones.Resolve(ctx.SourceTimezone);

        var ex = Assert.Throws<MappingValidationException>(() =>
            OrderStatsMapper.Map(dto, "SBER", ctx, tz, rowIndex: 17));

        Assert.Equal("mapping_validation", ex.ErrorCategory);
        Assert.Equal(MappingCategories.OrderStats, ex.Category);
        Assert.Equal("SBER", ex.SecId);
        Assert.Equal(17, ex.RowIndex);
    }
}
